...
select avg(length), floor(avg(length)/60) as hour, 
round(avg(length) - (60*floor(avg(length)/60))) as minute
from film;
...
select *, month(rental_date), weekday(rental_date), if(weekday(rental_date) <= 4, 'workday', 'weekend')
from rental
;
...
# Please note that even if there are currently no null values in the rental duration column, the query should still be written to handle such cases in the future. Hint: look for the IFNULL() function.
SELECT inventory_id, IFNULL(rental_date, 'Not Available') AS rental_date
FROM rental
ORDER BY inventory_id;

#4
SELECT CONCAT(first_name, ' ', last_name) AS customer_name, SUBSTRING(email, 1, 3) AS email_prefix
FROM customer
